...
**Développeur** : Elodie ATANA (Ingénieure IA & Lead Architect, Codorah)
...
L'application propose une galerie d'images préchargées, mais permet également l'importation de photos. Grâce à **`localforage`**, ces photos volumineuses sont redimensionnées silencieusement puis enregistrées dans la base de données `IndexedDB` du navigateur de l'utilisateur.
...
* Le message est encodé en **Base64** via l'URL (ex: `?msg=SGVsbG8=`).
...
* Aucune base de données backend n'est utilisée pour cet échange, tout est sécurisé de pair-à-pair côté client.

### 6. 🔀 Auto-Mélange Dynamique (Auto-Shuffle)
L'expérience utilisateur du partage est poussée au maximum : si l'application détecte qu'un joueur a ouvert un lien de défi contenant un message mystère, dès que l'animation du logo de démarrage se termine, **l'application sélectionne l'image reçue et mélange automatiquement les tuiles sous les yeux du joueur** au niveau de difficulté (ex: 5x5) dicté par le lien (`?lvl=5`). Le joueur peut commencer à glisser les tuiles immédiatement.
...
* **Page Paramètres Native** : Conçue comme une vraie page d'application système qui glisse du bas de l'écran, avec des interrupteurs iOS pour régler le mode de thème et les sons.

---

## 📂 Architecture des Fichiers Clés

* `src/App.jsx` : Contient l'intégralité de la logique de jeu, des cycles de vie React (`useEffect`, `useState`), du moteur de Canvas HD, du hachage de message et des requêtes LocalForage.
...
* `vite.config.js` : Configuration du bundler avec intégration du plugin PWA et définitions des règles Workbox pour forcer le cache global hors-ligne.
...
# 2. Installer les paquets (React, Vite, LocalForage)
npm install

# 3. Lancer le serveur de développement
npm run dev

# 4. Compiler pour la production / déploiement (génère le dossier /dist optimisé)
npm run build
```

## 🔐 Politique de Confidentialité & Sécurité
Application **Zéro-Tracking**. Aucune donnée personnelle, aucune image téléchargée, ni aucun message secret n'est envoyé vers internet. Toute l'intelligence (Redimensionnement, Sauvegarde, Décryptage) s'exécute côté-client directement dans la carte mémoire RAM et Sandbox du téléphone ou de l'ordinateur de l'utilisateur final.
